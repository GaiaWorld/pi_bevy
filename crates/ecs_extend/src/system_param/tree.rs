...
impl<'w> EntityTree<'w> {
    ...
    pub fn recursive_iter(&self, node_children_head: Entity) -> RecursiveIterator<EntityTree<'w>> {
        ...
        let len = if head.is_null() { 0 } else { 1 };
        RecursiveIterator {
            inner: RecursiveIterator1::new(self, head, len),
        }
    }
}
...
pub struct TreeStorageMut<'w> {
    ...
    // command: Commands<'w>, // 用于插入Root组件
                           // layer_notify: EventWriter<'w, ComponentEvent<Changed<Layer>>>, // 用于通知Layer修改
}

impl pi_world::prelude::SystemParam for EntityTreeMut<'_> {
    type State = (
        <Query<'static, &'static mut Layer> as pi_world::prelude::SystemParam>::State,
        <Query<'static, &'static mut Up> as pi_world::prelude::SystemParam>::State,
        <Query<'static, &'static mut Down> as pi_world::prelude::SystemParam>::State,
        // <Commands<'static> as pi_world::prelude::SystemParam>::State,
    ...
    fn init_state(
        ...
        (
            <Query<'static, &'static mut Layer> as pi_world::prelude::SystemParam>::init_state(
                world,
                system_meta,
            ),
            <Query<'static, &'static mut Up> as pi_world::prelude::SystemParam>::init_state(
                world,
                system_meta,
            ),
            <Query<'static, &'static mut Down> as pi_world::prelude::SystemParam>::init_state(
                world,
                system_meta,
            ),
            // <Commands<'static, 'static> as pi_world::prelude::SystemParam>::init_state(
            //     world,
            //     system_meta,
            // ),
    ...
    fn get_param<'world>(
        ...
        EntityTreeMut {
            tree: Tree::new(TreeStorageMut {
                layer_query: <Query<'world, &'static mut Layer> as SystemParam>::get_param(
                    world,
                    system_meta,
                    &mut state.0,
                ),
                up_query: <Query<'world, &'static mut Up> as SystemParam>::get_param(
                    world,
                    system_meta,
                    &mut state.1,
                    // change_tick,
                ),
                down_query: <Query<'world, &'static mut Down> as SystemParam>::get_param(
                    world,
                    system_meta,
                    &mut state.2,
                ),
                // command: todo!(),
                // command: <Commands<'world> as SystemParam>::get_param(
                //     &mut state.3,
                //     system_meta,
                //     world,
                // ),
                // layer_notify:
                //     <EventWriter<'w, ComponentEvent<Changed<Layer>>> as SystemParam>::get_param(
                //         &mut state.4,
                //         system_meta,
                //         world,
                //         change_tick,
    ...
    // fn archetype_depend(
    //     world: &pi_world::world::World,
    //     system_meta: &pi_world::system::SystemMeta,
    //     state: &Self::State,
    //     archetype: &pi_world::archetype::Archetype,
    //     result: &mut pi_world::archetype::ArchetypeDependResult,
    // ) {
    // }
    ...
    //     system_meta: &pi_world::system::SystemMeta,
    //     state: &Self::State,
    //     res_tid: &std::any::TypeId,
    //     res_name: &std::borrow::Cow<'static, str>,
    //     single: bool,
    //     result: &mut pi_world::archetype::Flags,
    // ) {
    // }

    // fn align(world: &pi_world::world::World, system_meta: &pi_world::system::SystemMeta, state: &mut Self::State) {}
    ...
    //     <Query<'static, 'static, &'static mut Layer> as bevy_ecs::system::SystemParam>::apply(
    //         &mut state.0,
    //         system_meta,
    //         world,
    //     );
    //     <Query<'static, 'static, &'static mut Up> as bevy_ecs::system::SystemParam>::apply(
    //         &mut state.1,
    //         system_meta,
    //         world,
    //     );
    //     <Query<'static, 'static, &'static mut Down> as bevy_ecs::system::SystemParam>::apply(
    //         &mut state.2,
    //         system_meta,
    //         world,
    //     );
    //     <Commands<'static, 'static> as bevy_ecs::system::SystemParam>::apply(
    //         &mut state.3,
    //         system_meta,
    //         world,
    //     );
    ...
    //     EntityTreeMut {
    //         tree: Tree::new(TreeStorageMut {
    //             layer_query: <Query<'w, 's, &'static mut Layer> as SystemParam>::get_param(
    //                 &mut state.0,
    //                 system_meta,
    //                 world,
    //                 change_tick,
    //             ),
    //             up_query: <Query<'w, 's, &'static mut Up> as SystemParam>::get_param(
    //                 &mut state.1,
    //                 system_meta,
    //                 world,
    //                 change_tick,
    //             ),
    //             down_query: <Query<'w, 's, &'static mut Down> as SystemParam>::get_param(
    //                 &mut state.2,
    //                 system_meta,
    //                 world,
    //                 change_tick,
    //             ),
    //             command: <Commands<'w, 's> as SystemParam>::get_param(
    //                 &mut state.3,
    //                 system_meta,
    //                 world,
    //                 change_tick,
...
impl<'w> EntityTreeMut<'w> {
    ...
    /// 迭代指定节点的所有递归子元素
    pub fn recursive_iter(
        &self,
        node_children_head: Entity,
    ) -> RecursiveIterator<TreeStorageMut<'w>> {
        RecursiveIterator {
            inner: self.tree.recursive_iter(TreeKey(node_children_head)),
        }
    }
}
...
impl<'w> Storage<TreeKey> for TreeStorageMut<'w> {
    fn get_up(&self, k: TreeKey) -> Option<&Up1<TreeKey>> {
        unsafe {
            transmute(match self.up_query.get(k.0) {
                Ok(r) => Some(r),
                _ => None,
            })
        }
    ...
    fn get_layer(&self, k: TreeKey) -> Option<&Layer1<TreeKey>> {
        unsafe {
            transmute(match self.layer_query.get(k.0) {
                Ok(r) => Some(r),
                _ => None,
            })
        }
    ...
    fn get_down(&self, k: TreeKey) -> Option<&Down1<TreeKey>> {
        unsafe {
            transmute(match self.down_query.get(k.0) {
                Ok(r) => Some(r),
                _ => None,
            })
        }
