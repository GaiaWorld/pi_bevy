...
use bevy_ecs::{
    ...
    query::{WorldQuery, Changed, Added, Or},
...
#[inline]
pub fn marked_dirty<'w, 's, 'a, T: Eq + Clone>(id: Entity, v: T, dirty_mark_list: &'a mut DirtyMark, dirty: &'a mut LayerDirty1<T>, id_tree: &EntityTree) {
...
pub fn marked<'w, 's, 'a, T: Eq + Clone>(id: Entity, v: T, dirty_mark_list: &'a mut DirtyMark, dirty: &'a mut LayerDirty1<T>, layer: usize) {
	if layer != 0 {
...
impl<'w, 's, F: Dirty> LayerDirty<'w, 's, F> where for<'a, 'b> <<F as Dirty>::EventReaderState as SystemParamFetch<'a, 'b>>::Item: EventList {
	...
	pub fn init(&mut self) {
		...
		for id in self.event_reader.iter() {
			marked_dirty(*id, *id, &mut self.dirty_mark, &mut self.layer_list, &self.entity_tree)
		}
		self.is_init = true;
	}
}
