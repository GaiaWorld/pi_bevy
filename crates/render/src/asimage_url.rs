...
const TARGET: &'static str = "asimage:://"; // 渲染目标类型的target， 其他类型TODO
...
pub fn entity_to_asimage_url(entity: Entity) -> String {
    format!("{}{}v{}", TARGET, entity.index(), entity.data().version())
}
...
pub fn load_from_asimage_url(url: &str, query: &Query<(OrDefault<RenderTarget>, OrDefault<GraphId>)>) -> Result<Option<(ShareTargetView, GraphId, Entity)>, LoadError> {
    ...
    let r = url[TARGET.len()..].to_string();
    let mut r = r.split("v");
    ...
    let entity = Entity::from(KeyData::from_ffi((version as u64) << 32 + index as u64));
